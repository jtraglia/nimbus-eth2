  ...
  block:
    ...
    while true:
      ...
      if attempts >= retries:
        ...
        await sleepAsync(chronos.milliseconds(retryDelayValue))
        retryDelayValue *= 2
...
proc exchangeConfigWithSingleEL(m: ELManager, connection: ELConnection) {.async.} =
  ...
  let
    ourConf = TransitionConfigurationV1(
      ...
      terminalBlockHash: m.depositsChain.cfg.TERMINAL_BLOCK_HASH,
...
proc connect(connection: ELConnection,
  ...
  let web3 = await establishEngineApiConnection(connection.engineUrl)
  if web3.isErr:
    return err web3.error

  connection.web3 = some web3.get

  if depositContractAddress.isSome:
    connection.depositContract = some web3.get.contractSender(
      DepositContract, depositContractAddress.get)

  return ok()

proc resetConnection(connection: var ELConnection,
...
proc runDbMigrations*(m: ELManager, rpcClient: RpcClient) {.async.} =
  ...
  if oldSnapshot.isSome:
    let
      ...
      blk = awaitWithRetries(rpcClient.getBlockByHash(hash),
                             web3RequestsTimeout)
    ...
    let blk = try:
      awaitWithRetries(getBlock(rpcClient, m.depositContractDeployedAt),
                       web3RequestsTimeout)
...
proc findTerminalBlock(rpcClient: RpcClient,
  ...
  proc next(x: BlockObject): Future[BlockObject] {.async.} =
    ...
    if key in cache:
      return cache[key]
    # Not cached, fetch.
    let value = awaitWithRetries(rpcClient.getBlockByNumber(key),
                                 web3RequestsTimeout)
    cache[key] = value
    return value
  ...
  var
    a = awaitWithRetries(rpcClient.eth_getBlockByNumber("latest", false),
                         web3RequestsTimeout)
    b = await next(a)
...
proc startDepositsSyncingLoop(m: ELManager) {.async.} =
  ...
  while true:
    try:
      ...
      await m.runDbMigrations(rpcClient)
